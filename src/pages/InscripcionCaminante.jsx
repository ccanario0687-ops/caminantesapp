...
import { CheckCircle, AlertCircle, User, Loader2, X, Check, FileText, Globe, MapPin } from "lucide-react";
import { formatCedula, toTitleCase } from "@/utils/formatters";
import MobileSelect from "@/components/MobileSelect";

// 🌐 BASE DE DATOS ECLESIÁSTICA MULTIPAÍS COMPLETA EN CASCADA
...
const EMPTY = {
  ...
  pais: "República Dominicana", provincia: "", diocesis: "", parroquia: "", padrino_madrina: "", telefono_padrino: "", direccion: "",
  calle: "", sector: "", municipio: "", ocupacion: "",
...
const inp = "w-full border border-amber-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 bg-white font-medium";
...
export default function InscripcionCaminante() {
  ...
  // 📏 UNIDADES DINÁMICAS DESDE CONFIGURACIÓN
  const unidadPeso = config?.unidad_peso || "kg";
  const unidadEstatura = config?.unidad_estatura || "cm";
  ...
  const set = (k, v) => {
    ...
    if (k === "telefono" || k === "telefono_padrino" || k === "telefono_emergencia") val = formatTelefono(v);
    setForm(f => ({ ...f, [k]: val }));
  ...
  const validar = () => {
    ...
    if (isCampoObligatorio("edad", true) && (!form.edad || isNaN(Number(form.edad)) || Number(form.edad) < 1)) e.edad = "Ingresa una edad válida";
    ...
    if (isCampoObligatorio("talla_camisa", false) && !form.talla_camisa) e.talla_camisa = "Requerido";
    if (isCampoObligatorio("peso", false) && !form.peso_kg) e.peso_kg = "Requerido";
    if (isCampoObligatorio("estatura", false) && !form.talla_cm) e.talla_cm = "Requerido";
    return e;
  };
      ...
      <div className="min-h-screen bg-gradient-to-br from-amber-50 to-orange-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full text-center border border-amber-100">
          ...
          <div className="bg-amber-50 rounded-xl p-4 border border-amber-200 mb-4">
            <p className="text-amber-800 text-sm font-bold">✝ {nombreRetiro}</p>
            {config?.edicion && <p className="text-amber-700 text-xs mt-0.5 font-semibold">Retiro #{config.edicion}</p>}
            <p className="text-amber-600 text-xs mt-1 italic">{eslogan}</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 to-orange-50 py-8 px-4 font-sans text-slate-800">
      <div className="max-w-2xl mx-auto mb-6 text-center">
        ...
        <h1 className="text-2xl font-bold text-amber-900 tracking-tight" style={{ fontFamily: "Georgia, serif" }}>✝ {nombreRetiro}</h1>
        {config?.edicion && <p className="text-amber-700 text-sm font-semibold">Retiro #{config.edicion}</p>}
        <p className="text-amber-600 text-xs mt-1 tracking-widest uppercase font-medium">{eslogan}</p>
        <div className="mt-3 inline-flex items-center gap-2 bg-amber-100 text-amber-800 px-4 py-2 rounded-full shadow-xs">
          <User className="w-4 h-4" />
          <span className="text-sm font-bold">Inscripción de Caminante</span>
        </div>
      </div>

      <form onSubmit={handleSubmit} className="max-w-2xl mx-auto space-y-5">
        <Card title="Datos Personales del Caminante">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {isCampoVisible("edad") && (
              <div>
                ...
                {errores.edad && <p className="text-red-500 text-xs mt-1 font-semibold">{errores.edad}</p>}
              </div>
            )}

            {/* 🧬 GÉNERO HEREDADO DE CONFIGURACIÓN DE FORMA AUTOMÁTICA */}
            {isCampoVisible("genero") && (
              <div>
                <label className={lbl}>Género {isCampoObligatorio("genero", true) && "*"}</label>
                {generoFijo ? (
                  <div className={`${inp} bg-amber-50/80 font-bold text-amber-900 flex items-center justify-between border-amber-300`}>
                    <span>{generoFijo}</span>
                    <span className="text-amber-700 text-[11px] font-semibold">(Fijado por la comunidad)</span>
                  </div>
                ) : (
            ...
            <div className="md:col-span-2 mt-2 pt-3 border-t border-amber-100">
              <label className="block text-sm font-bold text-amber-900 mb-2 flex items-center gap-1.5">
            ...
            {/* 🛡️ PADRINO / MADRINA Y CONTACTO DE EMERGENCIA */}
            <div className="md:col-span-2 mt-2 pt-3 border-t border-amber-100">
              <label className="block text-sm font-bold text-amber-900 mb-2">Acompañamiento y Emergencia</label>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                  <label className={lbl}>Padrino / Madrina</label>
                  <input className={inp} value={form.padrino_madrina} onChange={e => set("padrino_madrina", e.target.value)} placeholder="Nombre del Padrino o Madrina" />
                </div>
                <div>
                  <label className={lbl}>Teléfono del Padrino (809-000-0000)</label>
                  <input className={inp} value={form.telefono_padrino} onChange={e => set("telefono_padrino", e.target.value)} placeholder="809-000-0000" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {isCampoVisible("contacto_emergencia") && (
                  <div>
                    <label className={lbl}>Contacto de Emergencia {isCampoObligatorio("contacto_emergencia", true) && "*"}</label>
            ...
            {/* CAMPOS PERSONALIZADOS CREATIVOS */}
            <RenderCamposPersonalizados configCamposMap={configCamposMap} form={form} setForm={setForm} errores={errores} lbl={lbl} inp={inp} />
          </div>
        </Card>

        {/* 🏥 COTEJO SÍ / NO PARA NECESIDADES MÉDICAS Y UNIDADES DINÁMICAS */}
        <Card title="Datos Físicos y Médicos">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {isCampoVisible("peso") && (
              <div>
                <label className={lbl}>Peso ({unidadPeso}) {isCampoObligatorio("peso") && "*"}</label>
                <input type="number" min="20" max="300" className={inp} value={form.peso_kg} onChange={e => set("peso_kg", e.target.value)} placeholder={`Ej: 70 ${unidadPeso}`} />
              </div>
            )}
            {isCampoVisible("estatura") && (
              <div>
                <label className={lbl}>Estatura ({unidadEstatura}) {isCampoObligatorio("estatura") && "*"}</label>
                <input type="number" min="50" max="300" className={inp} value={form.talla_cm} onChange={e => set("talla_cm", e.target.value)} placeholder={`Ej: 170 ${unidadEstatura}`} />
              </div>
            )}
            ...
            {/* COTEJO DE SALUD CON COTEJO SÍ / NO */}
            {isCampoVisible("necesidades_medicas") && (
              <div className="md:col-span-2 pt-2 border-t border-amber-100">
                <label className="block text-sm font-bold text-amber-900 mb-2">¿Padece alguna enfermedad, alergia o condición médica? {isCampoObligatorio("necesidades_medicas") && "*"}</label>
                ...
                {tieneNecesidadMedica === "SI" && form.necesidades_medicas && (
                  <div className="mt-3 bg-amber-50 border border-amber-200 rounded-xl p-3 flex items-center justify-between text-xs text-amber-900">
                    <p className="font-semibold"><strong>Detalle:</strong> {form.necesidades_medicas}</p>
                    <button type="button" onClick={() => setModalMedicaOpen(true)} className="text-amber-700 font-bold underline ml-2">
                      Editar
                    </button>
                  </div>
                )}
        ...
        <button type="submit" disabled={enviando}
          className="w-full bg-amber-800 hover:bg-amber-900 text-white py-3.5 rounded-xl font-bold text-base transition-colors shadow-lg flex items-center justify-center gap-2">
      ...
      {modalMedicaOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-5 space-y-4 border border-amber-100">
            <div className="flex items-center justify-between border-b pb-2">
              <h3 className="font-bold text-amber-900 text-sm flex items-center gap-2">
                <FileText className="w-4 h-4 text-amber-700" /> Detalle de Condición Médica / Alergias
              </h3>
            ...
            <textarea
              ...
              className="w-full border border-amber-200 rounded-xl p-3 text-sm focus:ring-2 focus:ring-amber-400 focus:outline-none bg-white font-medium"
            />
            <div className="flex justify-end gap-2">
              ...
              <button type="button" onClick={guardarSaludModal} className="px-4 py-2 bg-amber-800 hover:bg-amber-900 text-white rounded-xl text-xs font-bold shadow-xs">Guardar</button>
            </div>
          </div>
        </div>
      )}
...
function Card({ title, children }) {
  return (
    <div className="bg-white rounded-xl shadow-xs border border-amber-100 p-5">
